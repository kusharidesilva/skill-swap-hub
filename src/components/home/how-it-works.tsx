...
export default function HowItWorksSection() {
  return (
    ...
    <section id="how-it-works" className="ssh-section-soft bg-white py-16 sm:py-24 scroll-mt-20">
      <div className="relative z-10 mx-auto max-w-6xl px-6">
        ...
        <div className="mt-14 grid gap-8 md:grid-cols-3">
          {steps.map((step, index) => (
            <div
              key={step.title}
              className="ssh-card ssh-step-card group relative rounded-2xl border border-emerald-100/60 bg-[#f4fbf7]/40 p-8 text-center shadow-xs transition-all duration-300 hover:scale-[1.03] hover:bg-white hover:border-emerald-300/80 hover:shadow-md"
            >
              <span className="ssh-step-index mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#e6fcf4] text-xl font-bold text-[#0f8a6b] shadow-xs group-hover:scale-110 transition-transform duration-300">
                {index + 1}
              </span>
