...
function CompletedView({ requests }: { requests: RequestData[] }) {
  ...
  return (
    <div className="grid w-full max-w-[1200px] gap-3 md:grid-cols-2 xl:grid-cols-3">
      {requests.length > 0 ? (
        requests.map((item) => {
          const hasBuyerReview = Boolean(item.review);
          const hasProviderReview = Boolean(item.providerReview);

          return (
            <article
              key={item.id}
              className="w-full rounded-[18px] border border-slate-200 bg-white p-3.5 shadow-[0_10px_22px_rgba(15,23,42,0.04)]"
            >
              <div className="flex items-start justify-between gap-2.5 border-b border-slate-100 pb-2.5">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400">
                    {item.buyerAccountType === "non-student"
                      ? "Non-student Buyer"
                      : item.buyerUniversity || "Student Buyer"}
                  </p>
                </div>
              ...
              <div className="mt-3">
                ...
                <p className="mt-1 line-clamp-1 text-sm font-bold text-[#1453c4]">
                  {item.title}
                </p>
              </div>

              {hasBuyerReview ? (
                <div className="mt-3 rounded-xl border border-amber-100 bg-amber-50/60 p-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-bold text-slate-700">Buyer&apos;s Review</p>
                    <p className="text-xs font-bold text-amber-500">
                      {"★".repeat(item.review!.rating)}
                      {"☆".repeat(5 - item.review!.rating)}
                    </p>
                  </div>
                  <p className="mt-1 line-clamp-2 text-xs italic leading-relaxed text-slate-600">
                    &ldquo;{item.review!.comment}&rdquo;
                  </p>
                </div>
              ) : (
                <div className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-xs italic text-slate-400">
                  Buyer has not reviewed yet.
                </div>
              )}

              {hasProviderReview ? (
                <div className="mt-2.5 rounded-xl border border-blue-100 bg-blue-50/60 p-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-bold text-blue-700">Your Review of Buyer</p>
                ...
                <button
                  onClick={() => setReviewingId(item.id)}
                  className="mt-2.5 w-full rounded-lg border border-dashed border-[#2f66e7] bg-blue-50/30 py-2 text-xs font-bold text-[#2f66e7] transition-colors hover:bg-blue-50"
                >
                  + Leave Review for Buyer
                </button>
              )}
